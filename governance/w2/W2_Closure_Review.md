...
Later additive Authority evidence closes framework conditions `AUTH-COND-001` and `AUTH-COND-004`. `AUTH-COND-002` still blocks production-proven claims. `AUTH-DEC-001` and `AUTH-DEC-002` provide the license, channel, and exact published `0.1.0-pilot.0` release decisions. `AUTH-DEC-003` records published `0.1.0-pilot.1`; `AUTH-DEC-005` approves `0.1.0-pilot.2`; new package bytes still require package-specific signatures. W2 technical PASS does not provide those approvals.
