...
Framework conditions `AUTH-COND-001` and `AUTH-COND-004` are closed by additive Authority evidence. New Skill, Plugin, npm, and GitHub release bytes require their own signatures and an exact-channel Authority decision. `AUTH-DEC-002` supplies that decision only for published `vibe-product-os@0.1.0-pilot.0`; `AUTH-DEC-003` approves only published `0.1.0-pilot.1`; and `AUTH-DEC-005` approves only `0.1.0-pilot.2` on npm `pilot` after exact signature verification. `AUTH-COND-002` blocks claims that the framework or Skill is production-proven until a real-project pilot supplies accepted evidence. W1 and W2 technical completion do not authorize distribution or close that claim boundary.
