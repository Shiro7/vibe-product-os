...
const helpText = `Vibe Product OS ${identity.package_version} — INTERNAL ALPHA
...
Safety:
  ...
  This alpha is not authorized for npm publication or other external distribution.
`;
...
async function run(args) {
  ...
  if (command === 'verify-release') {
    ...
    if (options.json) printJson(report);
    else process.stdout.write(`${report.result} — bytes ${report.byte_identity}, publisher ${report.publisher_identity}, external distribution NOT AUTHORIZED\n`);
