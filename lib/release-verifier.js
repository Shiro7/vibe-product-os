...
function verifyRelease(options = {}) {
  ...
  const signaturesVerified = results.every((entry) => entry.signature_identity === 'VERIFIED');
  const signatureFailure = results.some((entry) => !['VERIFIED', 'NOT_CONFIGURED'].includes(entry.signature_identity));
  ...
  return {
    ...
    ready_for_authority_signing: bytesVerified && !signaturesVerified,
    external_distribution_authorized: false,
    external_distribution_ready: false,
    blockers: [identity.external_distribution_blocker, identity.production_claim_blocker],
